class MegaTicTacToe {
    ...
    isValidMove(miniGridIndex, cellIndex) {
        ...
        // Check if the mini-grid is already won
        if (this.miniGridWinners[miniGridIndex] !== null) return false;
        
        // If there's an active mini-grid and it's not the current one
        if (this.activeMiniGrid !== null) {
            // If the active mini-grid is already won, allow any move
            if (this.miniGridWinners[this.activeMiniGrid] !== null) {
                return true;
            }
            // Otherwise, only allow moves in the active mini-grid
            if (this.activeMiniGrid !== miniGridIndex) return false;
        }
